...
import { BrainCircuit, User, Mail, Lock, Loader2, ArrowRight, Eye, EyeOff } from 'lucide-react';
...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (result.success) {
      toast.success('Registration successful! Welcome to Sugan Resume Analyzer.');
      navigate('/dashboard');
    } else {
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="glass-panel p-8 rounded-2xl border border-dark-800/80 shadow-2xl">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-3 rounded-lg glass-input text-sm ${
                    errors.name ? 'border-red-500/80 focus:border-red-500 focus:ring-red-500/10' : ''
                  }`}
                  {...registerField('name')}
                />
              </div>
              {errors.name && (
                <p className="text-xs text-red-400 mt-1.5 font-medium">{errors.name.message}</p>
              )}
            </div>
          ...
          <p className="text-sm text-dark-400 text-center mt-6">
            ...
            <Link to="/login" className="text-brand-400 hover:text-brand-350 font-semibold transition-colors">
              Sign In
            </Link>
          </p>
        </div>
